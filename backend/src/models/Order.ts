...
class Order extends Mongo {
    constructor(database: Database, private order?: OrderT) {
        super(database, COLLECTION, order);
    }
    ...
    async read(condition?: ConditionI) {
        ...
        const model = await collection.find(condition?.where).sort({date: 1, time: 1});
